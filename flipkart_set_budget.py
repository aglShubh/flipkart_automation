...
try:
    ...
    filename = "Flipkart_Set_budget_PCA" + str(now) + ".log"
    logging.basicConfig(
        filename=filename,
        filemode="w",
...
def setOrUpdateBudget():
    for camp in actionalble_data_list:
        ...
        if action == "set_budget":
            bid_url = f"https://advertising.flipkart.com/ad-account/campaigns/{seg.lower()}/{campaign_id}/edit?baccount=RSAUFLMCSZ&aaccount={account}"
            browser.get(bid_url)
            time.sleep(3)
            removePopUpBox()
            time.sleep(3)
            ...
            if camp["segment"] == "pla":
                clickContinue()
                time.sleep(2)
                BudgetBoxPLA(set_val=set_value)
                time.sleep(2)
                finalContinueBtn()
                time.sleep(3)
                submit(
