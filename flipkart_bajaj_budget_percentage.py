...
try:
    ...
    filename = "Flipkart_bajaj_budget_percentage_" + str(now) + ".log"
    logging.basicConfig(
        filename=filename,
        filemode="w",
...
def alchemy_connection():
    engine = create_engine(
        "mysql+pymysql://rbuser:Agl325Umn1d@43.204.245.132:2499/bajaj_media_automation"
    )
    return engine
...
DATABASE_NAME = "bajaj_media_automation"
...
def getFlipkartCampaignData():
    ...
    query = """SELECT campaign_id, campaign_status, campaign_budget_type, campaign_budget, platform_id, account_id FROM bajaj_media_automation.flipkart_supermart_capaigns where campaign_id is not Null """
...
def insertCrawlId():
    ...
    crawl_cursor.execute(
        "select max(crawl_id) as mx from bajaj_media_automation.flipkart_campaign_budget_spend;"
    )
    crawl_id = crawl_cursor.fetchone()
    crawl_id = crawl_id["mx"]
    logger.info(f"Fetched crawl id: {crawl_id}")
    if crawl_id is None:
        crawl_id = "1"
    else:
...
logger.info(f"return crawl_id: {crawl_id}")
...
def getBudgetSpendPercentage():
    ...
    for flip_camp in flipkart_campaign_list:
        ...
        campaign_url = f"https://advertising.flipkart.com/ad-account/campaigns?baccount={account_id}&aaccount={platform_id}"
        ...
        try:
            ...
            try:
                query = """
                insert into bajaj_media_automation.flipkart_campaign_budget_spend (campaign_id, campaign_budget, campaign_status, budget_type, spend_percentage, crawl_id, created_at) values(%s,%s,%s,%s,%s,%s,%s)"""
                cursor = db_connection.cursor()
                cursor.execute(
                    query,
                    (
            ...
            try:
                ...
                logger.error(
                    f"Error inserting data for campaing id {campaign_id} : {repr(_e)}"
                )
            pass
        except Exception as _e:
