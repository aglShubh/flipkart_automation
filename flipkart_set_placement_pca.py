...
try:
    ...
    filename = "Flipkart_set_placement_pca" + str(now) + ".log"
    logging.basicConfig(
        filename=filename,
        filemode="w",
...
def getActionableData():
    ...
    query = """SELECT action_status,
        ...
        top_of_browse_page, 
        rest_of_browse_page
        FROM automation.rpa_action where action_status = 1 and (segment ='PCA' or segment = 'pca');"""
...
def setPlacementPca():
    ...
    for act in actionalble_data_list:
        ...
        pca_camp_url = f"https://advertising.flipkart.com/ad-account/campaigns/{segment.lower()}/{campaign_id}/edit?baccount=RSAUFLMCSZ&aaccount={vanish_account}&step=1"
        browser.get(pca_camp_url)
        time.sleep(5)
