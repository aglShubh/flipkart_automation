...
try:
    ...
    filename = "Flipkart_Set_bid_PLA" + str(now) + ".log"
    logging.basicConfig(
        filename=filename,
        filemode="w",
...
#         time.sleep(3)
#         budget_box.click()
#         val: any = budget_box.get_attribute("value")
#         logger.info(f"current val of budget is : {val}")
#         budget_box.send_keys(Keys.CONTROL + "a")
#         budget_box.send_keys(Keys.DELETE)
#         time.sleep(2)
#         budget_box.send_keys(set_val)
...
def setOrUpdateBid():
    for camp in actionalble_data_list:
        ...
        seg = camp["segment"]
        print(action)
        if action == "set_bid" and seg == "pla":
            bid_url = f"https://advertising.flipkart.com/ad-account/campaigns/{seg.lower()}/{campaign_id}/edit?baccount=RSAUFLMCSZ&aaccount={vanish_account}&step=1"
            browser.get(bid_url)
            time.sleep(3)
            removePopUpBox()
            time.sleep(3)
