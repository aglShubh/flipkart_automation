...
try:
    ...
    filename = "Flipkart_pca_campaign_action" + str(now) + ".log"
    logging.basicConfig(
        filename=filename,
        filemode="w",
...
def getActionableData():
    ...
    query = """SELECT action_status, segment, action_type, action, fsn_id, campaign_id, ad_group_id, set_value, platform_id, account_id FROM automation.rpa_action where action_status = 1 and action_type = 'creative';"""
...
# creative_url = f"https://advertising.flipkart.com/ad-account/campaigns/pca/SDOZV9W4NDR9/CFAKU1UBK72Z?baccount=RSAUFLMCSZ&aaccount=C18XVZJB4GD7"
# # creative_url = f"https://advertising.flipkart.com/ad-account/campaigns/{segment.lower()}/{campaign_id}/{ad_group_id}?baccount={account_id}&aaccount={platform_id}"
...
def pauseEnableCreative():
    try:
        creative_box = browser.find_element(
            By.XPATH,
            '//*[@id="app"]/div[1]/div[1]/div/div[2]/div/div[2]/div/div/div[2]/div[2]/div/table/tbody/tr/td[3]/div/span/div',
        )
        creative_box.click()
        time.sleep(3)
        logger.info(f"creative state changed")
    except Exception as _e:
        logger.error(f"Error changing state of creative: {repr(_e)}")


def changeCreativeState():
