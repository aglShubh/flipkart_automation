...
try:
    ...
    filename = "flipkart_add_fsn_pla" + str(now) + ".log"
    logging.basicConfig(
        filename=filename,
        filemode="w",
...
def uploadFsnProduct():
    try:
        ...
        upload_box = browser.find_element(
            By.XPATH,
            '//*[@id="app"]/div[1]/div[1]/div/div[2]/div/div[2]/div[1]/div[2]/div[3]/div[2]/div[1]/div[2]/div[2]/div[1]/div',
        )
        time.sleep(3)
        ...
        try:
            elm.send_keys(os.getcwd() + "/addFsnPla/pla_fsn.csv")
            time.sleep(3)
        except:
            os.mkdir("addFsnPla")
            elm.send_keys(os.getcwd() + "/addFsnPla/pla_fsn.csv")
...
def addFsnPla():
    for act in actionalble_data_list:
        ...
        if action == "add_fsn" and segment.lower() == "pla":
            pla_fsn_edit_url = f"https://advertising.flipkart.com/ad-account/campaigns/{segment.lower()}/{campaign_id}/edit?baccount={account_id}&aaccount={platform_id}"
            browser.get(pla_fsn_edit_url)
            time.sleep(5)
            try:
                removePopUpBox()
                time.sleep(2)
